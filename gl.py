...
DEFAULT_PROJECTS_UNIQUE_USERS_IDS_FILE="unique_users_ids"
...
def get_unique_user_logins_from_groups():
...
def get_summary_set():
    set_groups = get_unique_user_logins_from_groups()
...
def write_unique_users_ids_to_json():
    ...
    data = list(data)
    write_to_json(data, DEFAULT_PROJECTS_UNIQUE_USERS_IDS_FILE)

def get_specific_attribute_for_all_unique_users(attribute="username"):
    data = read_from_json(DEFAULT_PROJECTS_UNIQUE_USERS_IDS_FILE)
    attr_dict = {}
    for id in data:
        response = requests.get(f"{GITLAB_API_USERS_URL_BASE}/{id}")
        response_json = response.json()
        attr_dict[id] = response_json[attribute]
    return attr_dict

def get_specific_attributes_for_all_unique_users(*attributes):
    data = read_from_json(DEFAULT_PROJECTS_UNIQUE_USERS_IDS_FILE)
    attr_dict = {}
    for id in data:
        attrs_list = []
        ...
        for attr in attributes:
            attrs_list.append(response_json[attr])
        attr_dict[id] = attrs_list
    return attr_dict
...
# get_unique_user_logins_from_groups()
# write_unique_users_ids_to_json()
# print(get_specific_attribute_for_all_unique_users())
print(get_specific_attributes_for_all_unique_users("name", "username"))